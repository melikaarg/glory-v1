...
class GLORY(nn.Module):
    ...
    def forward(self, subgraphs, mapping_idx_list, candidate_news, candidate_entity, entity_mask, label=None):
        ...
        for i, subgraph in enumerate(subgraphs):
            ...
            mapping_idx_list[i] = torch.clamp(mapping_idx_list[i], max=x_encoded.size(0) - 1)
...
class GLORY(nn.Module):
    ...
    def forward(self, subgraphs, mapping_idx_list, candidate_news, candidate_entity, entity_mask, label=None):
        ...
        for i, subgraph in enumerate(subgraphs):
            ...
            batch_size, num_clicked, token_dim = mapping_idx_list[i].shape[0], mapping_idx_list[i].shape[1], \
